...
namespace GraphLib 
{
  ...
  template<typename V, typename E>
  std::vector<std::pair<int, E>> Vertex<V,E>::getEdges() { return _edges; }
  //----< reserve memory for, and initialize, static count >-----
  template<typename V, typename E>
  int Vertex<V,E>::count = 0;
  ...
  template<typename V, typename E>
  Vertex<V,E>::Vertex(V v, int id) : _v(v), _id(id), _mark(false) {}
  ...
  template<typename V, typename E>
  typename Vertex<V,E>::Edge& Vertex<V,E>::operator[](int i) { return _edges[i]; }
  
  //----< index const vertex's edges >---------------------------
  template<typename V, typename E>
  typename Vertex<V,E>::Edge Vertex<V,E>::operator[](int i) const { return _edges[i]; }
  ...
  //----< return vertex's id >-----------------------------------
  template<typename V, typename E>
  int& Vertex<V,E>::id() { return _id; }
  
  //----< return number of edges >-------------------------------
  template<typename V, typename E>
  int Vertex<V,E>::size() { return _edges.size(); }
  ...
  public:
    ...
    Vertex<V,E>& operator[](int i);
    Vertex<V,E> operator[](int i) const;
    void addVertex(Vertex<V,E> v);
    void addEdge(E eval, Vertex<V,E>& parent, Vertex<V,E>& child);
    int findVertexIndexById(int id);
	Vertex<V,E> findVertexByName(V vertex); //Finding Vertex By Name
	std::vector<Vertex<V, E>> getAdjList();
    int size();
    template<typename F>
    void dfs(Vertex<V,E>& v, F f);
	Vertex<V, E> findVertexById(int id);
  private:
    std::vector< Vertex<V,E> > adj;
    std::unordered_map<int, int> idMap; // id maps to graph index
    template<typename F>
    void dfsCore(Vertex<V,E>& v, F f);
  };
  
  template<typename V,typename E>
  std::vector<Vertex<V, E>> Graph<V,E>::getAdjList() { return adj; }
  ...
  template<typename V, typename E>
  typename Vertex<V,E>& Graph<V,E>::operator[](int i) { return adj[i]; }
 
  //----< index const graph's vertex collection >----------------
  template<typename V, typename E>
  typename Vertex<V,E> Graph<V,E>::operator[](int i) const { return adj[i]; }
  ...
  template<typename V, typename E>
  int Graph<V,E>::size() { return adj.size(); }

  //----< return index of vertex with specified id >-------------
  template<typename V, typename E>
  int Graph<V,E>::findVertexIndexById(int id)  {   return idMap[id];  }

  template<typename V, typename E>
  Vertex<V, E> Graph<V, E>::findVertexById(int id)
  {
  ...
  //Find vertex by name
  template<typename V,typename E>
  Vertex<V,E> Graph<V, E>::findVertexByName(V name)
  {
	  for (auto it = adj.begin(); it != adj.end(); ++it)
	  {
		  Vertex<V, E> v = *it;
  ...
  {
    int childIndex = findVertexIndexById(child.id());
    if(childIndex == adj.size())
      throw std::exception("no edge child");
    int parentIndex = findVertexIndexById(parent.id());
