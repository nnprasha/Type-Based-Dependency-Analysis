...
CodeAnalysisExecutive::CodeAnalysisExecutive() : xmlDepDefaultFilePath("../XmlResult/DependencyAnalysisResult.xml"),
xmlScDefaultPath("../XmlResult/StrongComponentsResult.xml")
{
...
void CodeAnalysisExecutive::InitialAnalysis(int argc, char* argv[], CodeAnalysisExecutive& exec)
{	
...
void CodeAnalysisExecutive::DemoReq1()
{
	std::cout << "\n\n==============================================\n";
...
void CodeAnalysisExecutive::DemoReq2()
{
	std::cout << "\n\n==============================================\n";
...
void CodeAnalysisExecutive::DemoReq3()
{
	std::cout << "\n\n==============================================\n";
	std::cout << "\tREQUIREMENT NUMBER 3";
	std::cout << "\n==============================================\n\n";
	std::cout << " -->All Packages that have been defined in the PURPOSE section have been included in the Project. ";
	std::cout << "\n \t-->TypeAnalyzer, DepAnal, StrongComponents, Display, NoSqlDB, TestExecutive!!";
	std::cout << "\n -->The project can be checked to verify this requirement.";
}
void CodeAnalysisExecutive::DemoReq4(TypeTable& tt,ScopeStack<ElementType>& s,Display& display)
{
	std::cout << "\n\n==============================================\n";
	std::cout << "\tREQUIREMENT NUMBER 4";
	std::cout << "\n==============================================\n\n";

	std::cout << " -->A TypeAnalysis Package is provided that contains the type table in the form of a map. (Requirement Number 3) ";
	...
	display.showTypeTable(tt);
}

void CodeAnalysisExecutive::DemoReq5(DepAnal& da, Display& display,std::string cmdArgm)
{
	std::cout << "\n\n==============================================\n";
	std::cout << "\tREQUIREMENT NUMBER 5";
	std::cout << "\n==============================================\n\n";
	std::cout << " -->A DepAnal Package is provided that analysis all the dependencies between files in a file collection. (Requirement Number 3) ";
	std::cout << "\n -->The folder path (provided as a command line argument) that contains the files to analyse is: " << FileSystem::Path::getFullFileSpec(cmdArgm);
	...
	display.showDependencyAnalysis(da);
...
void CodeAnalysisExecutive::DemoReq6(Display& display,DepAnal& da, StrongComponents<std::string, std::string>& sc)
{
	std::cout << "\n\n==============================================\n";
	std::cout << "\tREQUIREMENT NUMBER 6";
	std::cout << "\n==============================================\n\n";
	std::cout << " -->A StrongComponents package has been defined that analyses strong components on the dependency analysis result. (Requirement Number 3)";
...
void CodeAnalysisExecutive::DemoReq7(int argc, char* argv[], StrongComponents<std::string, std::string>& sc)
{
	std::cout << "\n==================================================\n";
	std::cout << "\tREQUIREMENT NUMBER 7";
	std::cout << "\n==================================================\n";

	std::cout << "\n Saving Dependency result to XML File";
	std::cout << "\n------------------------------------\n";

	std::cout << "\n --> Saving the dependency analysis to XML File : "<<xmlDepDefaultFilePath;
	db.ToXml(false, xmlDepDefaultFilePath);
	...
	std::cout << "\n---------------------------------------------\n";
	std::cout << "\n --> Saving strong components to the XML file : "<<xmlScDefaultPath;
	sc.ToXmlSC(xmlScDefaultPath);
	std::cout << "\n --> Strong components saved to the above mentioned file. It can be checked to validate this requirement.";
}

void CodeAnalysisExecutive::DemoReq8(int argc, char* argv[])
{
	std::cout << "\n===============================================\n";
	...
	std::cout << "\n --> Path to directory tree containing files to analyze: " << argv[1];
	std::cout << "\n --> List of file patterns to match for selection of files: ";
	for (int i = 2; i < argc; i++)
	{
	...
	std::string xmlDepFilePath = "";
	std::string xmlScFilePath = "";
	boolean defaultDepFlag = true;
	boolean defaultScFlag = true;
	for (int i = 0; i < argc; i++)
	{
		std::string key = std::string(argv[i]);
		if (key == "/xmlDep")
		{
			xmlDepFilePath = std::string(argv[i+1]);
			defaultDepFlag = false;
		}
		if (key == "/xmlSc")
		{
			xmlScFilePath = std::string(argv[i + 1]);
			defaultScFlag = false;
		}
	}
	if (defaultDepFlag) 
		std::cout << "\n\t  -->No argument present for dependency analysis result file in cmd line. Taking the default path (" << xmlDepDefaultFilePath << ")";		
	else {
		std::cout << "\n\t -->Argument Present in cmd line for dependency analysis result file. Path is: " << xmlDepFilePath;
		xmlDepDefaultFilePath = xmlDepFilePath;
	}		
	
	if (defaultScFlag)
		std::cout << "\n\t  -->No argument present for StrongComponent analysis result file in cmd line. Taking the default path: " << xmlScDefaultPath;
	else {
		std::cout << "\n\t -->Argument Present in cmd line for strong component analysis result file. Path is: " << xmlScFilePath;
		xmlScDefaultPath = xmlScFilePath;
	}
	if (!(FileSystem::Directory::exists(std::string(argv[1]))))
		throw "Specified directory tree of path doesnt exist. Please check command line arguments!!";
}

void CodeAnalysisExecutive::DemoReq9()
{

	std::cout << "\n\n===============================================\n";
	std::cout << "\t REQUIREMENT NUMBER 9";
	std::cout << "\n===============================================\n";
	std::cout << " -->All the above requirements have been demonstrated using an automated unit test executive suite.\n";
	std::cout << " -->The project CodeAnalyzer/Executive.cpp can be checked to verify this requirement!!";
	std::cout << "\n\n================== END OF PROJECT DEMONSTRATION ====================\n\n";
}
void CodeAnalysisExecutive::getFilesAndAnalyse(DepAnal& d)
{
	for (auto file : cppHeaderFiles())
	{		
		db=d.dependencyAnalsis(file);
	}
...
int main(int argc, char* argv[])
{
	...
	{		
		...
		exec.DemoReq8(argc,argv);
		exec.DemoReq4(tt,s,display);
		exec.DemoReq5(da, display,std::string(argv[1]));

		StrongComponents<V,E> sc(da.getDepMap());
		exec.DemoReq6(display,da,sc);
		exec.DemoReq7(argc,argv,sc);
		exec.DemoReq9();
  ...
  {
	  std::cout << "\n\n Caught exception in Executive::main: " + std::string(e) + "\n\n";
	  return 1;
  }
  return 0;
}
